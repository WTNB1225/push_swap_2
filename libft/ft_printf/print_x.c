...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/07 20:50:21 by wyuki             #+#    #+#             */
/*   Updated: 2025/05/28 20:58:30 by wyuki            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static size_t	print_hash_num_precision(unsigned int n, char *hex_char,
	t_spec *spec)
{
	size_t	printed_len;
	size_t	i;
	...
	{
		if (spec->precision > ft_strlen(hex_char))
			num_of_zero = spec->precision - ft_strlen(hex_char);
		while (i++ < num_of_zero)
			printed_len += putchar_fd('0', 1);
	}
	if (spec->is_hash && n != 0)
		printed_len += putstr_fd("0x", 1);
	printed_len += print_lower_hex(n);
	return (printed_len);
}
...
int	print_x(unsigned int n, t_spec *spec)
{
	...
	printed_len += print_hash_num_precision(n, hex_char, spec);
	if (spec->is_minus)
